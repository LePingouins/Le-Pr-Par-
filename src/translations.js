export const translations = {
  fr: {
    nav: {
      home: 'Accueil',
      products: 'Produits',
      about: 'Qui Suis-Je?',
      contact: 'Contact',
    },

    home: {
      hero: {
        title: 'Le Pré Paré',
        subtitle: 'Producteur Maraîcher',
        slogan: 'Cultivé avec amour et passion;\nde mon jardin à votre assiette.',
        tagline: 'Produits naturels sans engrais chimiques, sans pesticides et sans compromis.',
        cta: 'Découvrir nos produits',
        ctaAbout: 'En savoir plus',
      },
      features: [
        { icon: '🌿', title: 'Sans Pesticides', text: 'Production 100\u00a0% écologique, sans pesticides ni engrais chimiques.' },
        { icon: '🏡', title: 'Artisanal & Local', text: 'Directement de notre jardin de Senneville.' },
        { icon: '💚', title: 'Avec Amour', text: 'Chaque produit est cultivé avec passion et le plus grand soin.' },
        { icon: '💰', title: 'Prix Imbattable', text: 'Le meilleur rapport qualité-prix pour vous offrir le meilleur de la nature.' },
      ],
      story: {
        badge: 'Notre Histoire',
        title: 'Passionnée de la terre et de la table',
        text: 'Passionnée de jardinage, d\'alimentation et de cuisine, je rêve depuis longtemps d\'un projet qui mette en relation toutes les passions de ma vie. Aujourd\'hui je veux partager ce qui me rend heureuse et faire vivre une expérience au rapport qualité prix imbattable.',
        cta: 'En savoir plus sur moi',
      },
      categories: {
        title: 'Nos Propositions',
        subtitle: 'Sept gammes de produits cultivés et transformés avec soin, directement de la terre à votre cuisine.',
      },
      cta: {
        title: 'Prêt à commander?',
        text: 'Contactez-nous pour connaître les disponibilités et les prix de la saison.',
        btn: 'Nous contacter',
      },
    },

    products: {
      title: 'Nos Produits',
      subtitle: 'Cultivés et transformés avec amour et passion, sans compromis sur la qualité.',
      payment: {
        title: 'Modalité de Paiement',
        text: 'Nous acceptons le virement Interac.',
      },
      comingSoonLabel: 'À venir',
      availableLabel: 'Disponible',
      limitedLabel: 'Disponible en quantité limitée',
      contactForPrice: 'Prix sur demande',
      orderBtn: 'Commander',
      useLabel: 'Idéal pour',
      taxLabel: '+ taxes',
      seasons: {
        spring: 'Printemps',
        summer: 'Été',
        fall: 'Automne',
        winter: 'Hiver',
        'all-seasons': 'Toutes Saisons',
      },
      categories: [
        {
          id: 'plants',
          emoji: '🌱',
          name: 'Le Pré Paré — Les Plants',
          tag: 'available',
          limited: true,
          taxable: true,
          defaultSeason: 'spring',
          description: 'Cultivés avec amour et passion sans pesticides ni engrais chimiques.',
          detail: 'Nos plants sont élevés dans les meilleures conditions pour assurer une croissance saine et vigoureuse. Disponibles chaque printemps.',
          items: [
            {
              name: 'Tomate Early Girl',
              description: 'Rouge ferme et charnue — plant indéterminé',
              use: 'Passe-partout',
              price: '4$ l\'unité ou 3 pour 10$',
              season: ['spring', 'summer'],
              image: '/images/vegetable/Tomate_beefsteak.jpg',
            },
            {
              name: 'Tomate Beefsteak Nain',
              description: 'Grosse et charnue — plant nain déterminé',
              use: 'Sandwich',
              price: '4$ l\'unité ou 3 pour 10$',
              season: ['spring', 'summer'],
              image: '/images/vegetable/tomate_beefsteak.png',
            },
            {
              name: 'Tomate Roma',
              description: 'Rouge orangé, chair riche et charnue — plant déterminé',
              use: 'Sauce tomate, tomates séchées',
              price: '4$ l\'unité ou 3 pour 10$',
              season: ['spring', 'summer'],
              image: '/images/vegetable/Tomate_italienne.jpg',
            },
            {
              name: 'Tomate Yellow Pearshape',
              description: 'Petite, jaune sucrée, forme de poire — plant indéterminé',
              use: 'Frais, en salade',
              price: '4$ l\'unité ou 3 pour 10$',
              season: ['spring', 'summer'],
              image: '/images/vegetable/Tomate_yellow_pearshape.jpg',
            },
            {
              name: 'Tomate Sweetie',
              description: 'Cerise sucrée — plant indéterminé',
              use: 'Frais, en salade',
              price: '4$ l\'unité ou 3 pour 10$',
              season: ['spring', 'summer'],
              image: '/images/vegetable/Toamtes_sweeties.jpg',
            },
            {
              name: 'Tomate surprise',
              description: 'Variété surprise du jardin',
              use: 'À découvrir!',
              price: '2$ l\'unité',
              season: ['spring', 'summer'],
              image: '/images/vegetable/Plant_de_tomate_suprise.jpg',
            },
            {
              name: 'Poivron Golden California Wonder',
              description: 'Jaune, sucré',
              use: 'Passe-partout',
              price: '4$ l\'unité ou 3 pour 10$',
              season: ['spring', 'summer'],
              image: '/images/vegetable/poivron_jaune_california_wonder.jpg',
            },
            {
              name: 'Poivron Early California Wonder',
              description: 'Vert ferme, forme de cloche, devient rouge à maturité',
              use: 'Légumes farcis',
              price: '4$ l\'unité ou 3 pour 10$',
              season: ['spring', 'summer'],
              image: '/images/vegetable/Early_california_wonder.jpg',
            },
            {
              name: 'Poivron Surprise',
              description: 'Variété surprise du jardin',
              use: 'Passe-partout',
              price: '2$ l\'unité',
              season: ['spring', 'summer'],
              image: '/images/vegetable/Poivron_surprise.jpg',
            },
            {
              name: 'Bette à Carde Multicolore',
              description: 'Tiges colorées, feuilles tendres',
              use: 'Sauté, salade, et gratin',
              price: '2$ l\'unité',
              season: ['spring', 'summer', 'fall'],
              image: '/images/vegetable/betterave_a_carde.jpg',
            },
          ],
        },
        {
          id: 'flowers',
          emoji: '🌸',
          name: 'Le Pré Paré — Les Fleurs',
          tag: 'available',
          defaultSeason: 'spring',
          description: 'Des couleurs pour égayer votre jardin, cultivées avec le même soin que nos légumes.',
          detail: 'Nos plants de fleurs sont élevés dans les meilleures conditions pour assurer une floraison saine et généreuse. Disponibles chaque printemps.',
          items: [
            {
              name: 'Calendula Costa ou Bombon',
              description: 'Fleur officinale colorée, floraison abondante',
              price: 'Prix sur demande',
              season: ['spring', 'summer'],
            },
            {
              name: 'Lobelia Compacta Regatta',
              description: 'Petites fleurs cascadantes, idéale en suspension ou bordure',
              price: 'Prix sur demande',
              season: ['spring', 'summer'],
            },
            {
              name: 'Tagetes Patula Disco',
              description: 'Œillet d\'Inde compact aux fleurs dorées',
              price: 'Prix sur demande',
              season: ['spring', 'summer'],
            },
            {
              name: 'Viola Cornuta Colormax',
              description: 'Pensée cornue, disponible en plusieurs coloris',
              price: 'Prix sur demande',
              season: ['spring', 'summer'],
            },
            {
              name: 'Cosmos Bipinnatus Sensation',
              description: 'Grandes fleurs légères et aériennes, idéales en bouquets',
              price: 'Prix sur demande',
              season: ['spring', 'summer'],
            },
            {
              name: 'Gaillardia Grandiflora Arizona',
              description: 'Fleur vivace aux tons chauds, plusieurs coloris disponibles',
              price: 'Prix sur demande',
              season: ['spring', 'summer'],
            },
            {
              name: 'Geranium Zonal Fantasia',
              description: 'Géranium compact, plusieurs coloris disponibles',
              price: 'Prix sur demande',
              season: ['spring', 'summer'],
            },
            {
              name: 'Geranium Zonal Galaxy',
              description: 'Géranium aux fleurs bicolores et décoratives',
              price: 'Prix sur demande',
              season: ['spring', 'summer'],
            },
          ],
        },
        {
          id: 'dehydrated',
          emoji: '🍃',
          name: 'Les Incontournables Déshydratés',
          tag: 'available',
          limited: true,
          defaultSeason: 'all-seasons',
          description: 'Transformés avec amour et passion, ils sont indispensables en cuisine.',
          detail: 'Déshydratés à basse température pour conserver tous les nutriments et la saveur. Leur conservation est de longue durée tout naturellement.',
          items: [
            {
              name: 'Pépites d\'or — Ail Séché',
              description: 'Ail déshydraté à basse température pour conserver les nutriments',
              use: 'Passe-partout en cuisine',
              price: '6$',
              volume: '237 ml',
              image: '/images/vegetable/Pepite_d_or_seche.jpg',
            },
            {
              name: 'Pépites d\'or — Ail Séché aux Herbes et Piment',
              description: 'Ail séché mélangé à des herbes séchées et du piment',
              use: 'Marinades, vinaigrettes, plats relevés',
              price: '6$',
              volume: '237 ml',
              image: '/images/vegetable/Pépite_d_or_ail_séché_aux_herebes_et_piment.jpg',
            },
            {
              name: 'Mélange de Piments Fort',
              description: 'Piments forts déshydratés à basse température pour conserver les nutriments',
              use: 'Piments forts en cuisine',
              price: '5$',
              volume: '237 ml',
              image: '/images/vegetable/Mélange_de_piments_foret.jpg',
            },
            {
              name: 'LE Mélange',
              description: 'Assaisonnement passe-partout à base d\'ail, tomates séchées et mélange d\'herbes du jardin, relevé avec une pointe de piment',
              use: 'Passe-partout — viandes, légumes, pâtes',
              price: '6$',
              volume: '237 ml',
              image: '/images/vegetable/LE_mélange.jpg',
            },
          ],
        },
        {
          id: 'frozen',
          emoji: '❄️',
          name: 'Les Incontournables Congelés',
          tag: 'available',
          defaultSeason: 'all-seasons',
          description: 'Soigneusement Préparés pour vous, ils sont indispensables en cuisine.',
          detail: 'Blanchis, refroidis puis congelés au sommet de leur fraîcheur pour conserver tous les nutriments et la saveur. Pratiques et délicieux pour vos repas quotidiens.',
          items: [
            {
              name: 'Mélange Soupe',
              description: 'Carotte, oignon, patate douce, céleri, maïs',
              use: 'Soupes et potages',
              price: '5$',
              volume: '500 g',
              image: '/images/vegetable/Congelé_Mélange_a_soupe.jpg',
            },
            {
              name: 'Mélange Spaghetti',
              description: 'Carotte, oignon, poivron, céleri, ail',
              use: 'Sauces à pâtes',
              price: '5$',
              volume: '500 g',
              image: '/images/vegetable/Congelé_sauce_a_spag.jpg',
            },
            {
              name: 'Mélange Ratatouille',
              description: 'Poivron, courgette, oignon, aubergine, tomate, ail',
              use: 'Ratatouille, plats mijotés',
              price: '5$',
              volume: '500 g',
              image: '/images/vegetable/Congelé_ratatouille.jpg',
            },
            {
              name: 'Mélange Passe-Partout',
              description: 'Oignon, poivron, haricots, courge, ail',
              use: 'Passe-partout en cuisine',
              price: '5$',
              volume: '500 g',
              image: '/images/vegetable/Congelé_passe_partout.jpg',
            },
            {
              name: 'Bouillon de Légume',
              description: 'Infusé délicatement avec légumes et fines herbes',
              use: 'Base de soupes, risottos, sauces',
              price: '5$',
              volume: '946 ml',
              image: '/images/vegetable/Bouillon_de_légumes.jpg',
            },
            {
              name: 'Pesto de Fines Herbes',
              description: 'À base d\'un mélange d\'herbes avec noix, parmesan, huile et ail',
              use: 'Pâtes, tartines, marinades',
              price: '6$',
              volume: '237 ml',
              image: '/images/vegetable/Pesto.jpg',
            },
            {
              name: 'Beurre à l\'Ail',
              description: 'Beurre infusé à l\'ail du jardin',
              use: 'Pain, légumes, viandes grillées',
              price: '6$',
              volume: '237 ml',
              image: '/images/vegetable/Beurre_a_l_ail.jpg',
            },
            {
              name: 'Base Sauce Tomate',
              description: 'Sauce tomate maison',
              use: 'Pâtes, pizzas, mijotés',
              price: '8$',
              volume: '946 ml',
              image: '/images/vegetable/Sauce_tomate.jpg',
            },
            {
              name: 'Base Sauce Tomate et Poivrons',
              description: 'Sauce tomate enrichie de poivrons du jardin',
              use: 'Pâtes, pizzas, mijotés',
              price: '8$',
              volume: '946 ml',
              image: '/images/vegetable/Sauce_tomate_poivrons.jpg',
            },
          ],
        },
        {
          id: 'seasonal',
          emoji: '🥕',
          name: 'Légumes au Fil des Saisons',
          tag: 'available',
          limited: true,
          description: 'Sans pesticides ni engrais chimiques.',
          detail: 'Des légumes frais récoltés à pleine maturité. Livraison dans un rayon de 5 km de Senneville et de Sainte-Anne-de-Bellevue, frais de livraison de 5$ applicable, minimum de commande de 30$. Ramassage disponible sur place.',
          items: [
            {
              name: 'Le Sac Frais du Jour',
              description: 'Un mélange de légumes, verdures et fines herbes fraîchement récoltés',
              use: 'Repas de toute la semaine',
              price: '20$',
              season: ['summer', 'fall'],
              image: '/images/vegetable/Sac_de_légumes_frais.jpg',
            },
            {
              name: 'Fleur d\'Ail',
              description: 'Saisonnière — printemps',
              use: 'Sauté, légumes fermentés, pesto',
              price: '5$ la livre',
              season: 'spring',
              image: '/images/vegetable/Fleur_d_ail.jpg',
            },
            {
              name: 'Ail Frais',
              description: 'Ail du jardin récolté en été',
              use: 'Passe-partout en cuisine',
              price: 'Nous contacter',
              season: 'summer',
              image: '/images/vegetable/Ail_frais.jpg',
            },
            {
              name: 'Tomates de Fin de Saison',
              description: 'Tomates mûres récoltées en automne',
              use: 'Sauce, conserves, frais',
              price: 'Nous contacter',
              season: 'fall',
              image: '/images/vegetable/Tomates_de_fin_de_saison.jpg',
            },
          ],
        },
        {
          id: 'prepared',
          emoji: '🍲',
          name: 'La Popotte à PP — Plats Préparés',
          tag: 'soon',
          description: 'Cette catégorie sera développée sous peu.',
          detail: 'Nous travaillons sur cette nouvelle offre : des plats maison préparés avec nos propres légumes. Restez à l\'affût des nouveautés!',
          items: [],
        },
        {
          id: 'misc',
          emoji: '✨',
          name: 'Divers',
          tag: 'available',
          defaultSeason: 'all-seasons',
          description: 'Amendement naturel pour enrichir votre sol.',
          detail: 'Des produits complémentaires pour prendre soin de votre jardin.',
          items: [
            {
              name: 'Biochar',
              description: 'Amendement à effet prolongé pour enrichir et conditionner le sol',
              use: 'Jardinage, amélioration du sol',
              price: '1$ le litre',
              image: '/images/vegetable/Biochar.jpg',
              link: 'https://lasemaineverte.com/biochar',
            },
          ],
        },
      ],
    },

    about: {
      title: 'Qui Suis-Je?',
      subtitle: 'La femme derrière Le Pré Paré',
      name: 'Chantal Paré',
      role: 'Fondatrice & Productrice',
      bio: [
        'Titulaire d\'un certificat en horticulture et gestion des espaces verts je me spécialise depuis plus de 20 ans dans la création de potagers et dans l\'aménagement et l\'entretien de végétaux de toute sorte.',
        'Passionnée de jardinage, d\'alimentation et de cuisine, je rêve depuis longtemps d\'un projet qui mette en relation toutes les passions de ma vie.',
        'Mes expériences passées m\'ont apporté beaucoup de connaissances, m\'ont fait rencontrer des gens formidables et m\'ont fait participer à différents projets enrichissants.',
        'Aujourd\'hui je veux partager ce qui me rend heureuse et faire vivre une expérience audacieuse à vos papilles.',
        'Nous produisons de façon écologique et responsable.',
      ],
      values: {
        title: 'Nos Valeurs',
        items: [
          { icon: '🌎', title: 'Écologique & Responsable', text: 'Production sans pesticides ni engrais chimiques, dans le respect de la nature et de l\'environnement.' },
          { icon: '❤️', title: 'Passion & Amour', text: 'Chaque geste et chaque soin reflètent notre amour profond pour la terre, les plantes et l\'alimentation saine.' },
          { icon: '🤝', title: 'Communauté', text: 'Rencontrer des gens formidables et partager notre passion avec notre communauté locale.' },
          { icon: '⭐', title: 'Qualité', text: 'Offrir le meilleur rapport qualité-prix grâce à des produits frais, locaux et cultivés avec le plus grand soin.' },
        ],
      },
      contact: {
        title: 'Restons en contact',
        cta: 'Nous écrire',
      },
    },

    contact: {
      title: 'Contactez-Nous',
      subtitle: 'Nous sommes à votre écoute pour toute question, commentaire, commande ou collaboration.',
      info: {
        title: 'Informations',
        address: 'Adresse',
        addressValue: '25 rue Legault, Senneville, QC',
        phone: 'Téléphone',
        phoneValue: '514-691-0949',
        email: 'Courriel',
        emailValue: 'le.prepare1@gmail.com',
        social: 'Réseaux Sociaux',
        payment: 'Paiement',
        paymentValue: 'Virement Interac',
      },
      form: {
        title: 'Envoyez-Nous un Message',
        name: 'Votre nom',
        email: 'Votre courriel',
        subject: 'Sujet',
        message: 'Votre message',
        send: 'Envoyer le message',
        namePlaceholder: 'Marie Tremblay',
        emailPlaceholder: 'marie@exemple.com',
        subjectPlaceholder: 'Question sur les produits…',
        messagePlaceholder: 'Bonjour Chantal, je voudrais…',
        successMsg: 'Merci! Votre message a été envoyé.',
      },
    },

    footer: {
      tagline: 'De mon jardin à votre assiette.',
      quickLinks: 'Liens Rapides',
      contactTitle: 'Contact',
      rights: 'Tous droits réservés.',
      madeWith: 'Fait avec ❤️ au Québec',
    },
  },

  /* ── ENGLISH ──────────────────────────────────────────── */
  en: {
    nav: {
      home: 'Home',
      products: 'Products',
      about: 'About Me',
      contact: 'Contact',
    },

    home: {
      hero: {
        title: 'Le Pré Paré',
        subtitle: 'Market Gardener',
        slogan: 'Grown with love and passion;\nfrom my garden to your table.',
        tagline: 'Natural products — no chemical fertilizers, no pesticides, no compromises.',
        cta: 'Discover our products',
        ctaAbout: 'Learn more',
      },
      features: [
        { icon: '🌿', title: 'Pesticide-Free', text: '100% ecological production — no pesticides or chemical fertilizers.' },
        { icon: '🏡', title: 'Artisanal & Local', text: 'Directly from our garden in Senneville.' },
        { icon: '💚', title: 'Made with Love', text: 'Every product is cultivated with passion and the greatest care.' },
        { icon: '💰', title: 'Unbeatable Value', text: 'The best quality-to-price ratio — the very best nature has to offer.' },
      ],
      story: {
        badge: 'Our Story',
        title: 'Passionate about the land and the table',
        text: 'Passionate about gardening, nutrition, and cooking, I have long dreamed of a project that brings all my passions together. Today I want to share what makes me happy and offer an experience with unbeatable value.',
        cta: 'Learn more about me',
      },
      categories: {
        title: 'Our Offerings',
        subtitle: 'Seven product ranges grown and transformed with care, straight from the earth to your kitchen.',
      },
      cta: {
        title: 'Ready to order?',
        text: 'Contact us to find out about seasonal availability and pricing.',
        btn: 'Contact us',
      },
    },

    products: {
      title: 'Our Products',
      subtitle: 'Grown and transformed with love and passion — no compromises on quality.',
      payment: {
        title: 'Payment Method',
        text: 'We accept Interac e-Transfer.',
      },
      comingSoonLabel: 'Coming soon',
      availableLabel: 'Available',
      limitedLabel: 'Available in limited quantities',
      contactForPrice: 'Price on request',
      orderBtn: 'Order',
      useLabel: 'Best for',
      taxLabel: '+ tax',
      seasons: {
        spring: 'Spring',
        summer: 'Summer',
        fall: 'Fall',
        winter: 'Winter',
        'all-seasons': 'All Seasons',
      },
      categories: [
        {
          id: 'plants',
          emoji: '🌱',
          name: 'Le Pré Paré — Seedlings & Plants',
          tag: 'available',
          limited: true,
          taxable: true,
          defaultSeason: 'spring',
          description: 'Grown with love and passion — no pesticides or chemical fertilizers.',
          detail: 'Our seedlings are raised under the best conditions to ensure healthy, vigorous growth. Available every spring.',
          items: [
            {
              name: 'Early Girl Tomato',
              description: 'Red, firm and fleshy — indeterminate plant',
              use: 'All-purpose',
              price: '$4 each or 3 for $10',
              season: ['spring', 'summer'],
              image: '/images/vegetable/Tomate_beefsteak.jpg',
            },
            {
              name: 'Dwarf Beefsteak Tomato',
              description: 'Large and fleshy — dwarf determinate plant',
              use: 'Sandwiches',
              price: '$4 each or 3 for $10',
              season: ['spring', 'summer'],
              image: '/images/vegetable/tomate_beefsteak.png',
            },
            {
              name: 'Roma Tomato',
              description: 'Orange-red, rich and fleshy — determinate plant',
              use: 'Tomato sauce, sun-dried tomatoes',
              price: '$4 each or 3 for $10',
              season: ['spring', 'summer'],
              image: '/images/vegetable/Tomate_italienne.jpg',
            },
            {
              name: 'Yellow Pearshape Tomato',
              description: 'Small, yellow, sweet, pear-shaped — indeterminate plant',
              use: 'Fresh, in salads',
              price: '$4 each or 3 for $10',
              season: ['spring', 'summer'],
              image: '/images/vegetable/Tomate_yellow_pearshape.jpg',
            },
            {
              name: 'Sweetie Cherry Tomato',
              description: 'Sweet cherry tomato — indeterminate plant',
              use: 'Fresh, in salads',
              price: '$4 each or 3 for $10',
              season: ['spring', 'summer'],
              image: '/images/vegetable/Toamtes_sweeties.jpg',
            },
            {
              name: 'Surprise Tomato',
              description: 'Mystery variety from the garden',
              use: 'Discover it yourself!',
              price: '$2 each',
              season: ['spring', 'summer'],
              image: '/images/vegetable/Plant_de_tomate_suprise.jpg',
            },
            {
              name: 'Golden California Wonder Pepper',
              description: 'Yellow, sweet',
              use: 'All-purpose',
              price: '$4 each or 3 for $10',
              season: ['spring', 'summer'],
              image: '/images/vegetable/poivron_jaune_california_wonder.jpg',
            },
            {
              name: 'Early California Wonder Pepper',
              description: 'Green, firm, bell-shaped — turns red at maturity',
              use: 'Stuffed peppers',
              price: '$4 each or 3 for $10',
              season: ['spring', 'summer'],
              image: '/images/vegetable/Early_california_wonder.jpg',
            },
            {
              name: 'Surprise Pepper',
              description: 'Mystery variety — garden surprise',
              use: 'All-purpose',
              price: '$2 each',
              season: ['spring', 'summer'],
              image: '/images/vegetable/Poivron_surprise.jpg',
            },
            {
              name: 'Rainbow Swiss Chard',
              description: 'Colourful stems, tender leaves',
              use: 'Sautéed, salads, and gratins',
              price: '$2 each',
              season: ['spring', 'summer', 'fall'],
              image: '/images/vegetable/betterave_a_carde.jpg',
            },
          ],
        },
        {
          id: 'flowers',
          emoji: '🌸',
          name: 'Le Pré Paré — Flowers',
          tag: 'available',
          defaultSeason: 'spring',
          description: 'Bursts of colour for your garden, grown with the same care as our vegetables.',
          detail: 'Our flower seedlings are raised under the best conditions to ensure healthy, abundant blooming. Available every spring.',
          items: [
            {
              name: 'Calendula Costa ou Bombon',
              description: 'Colourful medicinal flower, blooms abundantly',
              price: 'Price on request',
              season: ['spring', 'summer'],
            },
            {
              name: 'Lobelia Compacta Regatta',
              description: 'Small cascading flowers, ideal for hanging baskets or borders',
              price: 'Price on request',
              season: ['spring', 'summer'],
            },
            {
              name: 'Tagetes Patula Disco',
              description: 'Compact French marigold with golden flowers',
              price: 'Price on request',
              season: ['spring', 'summer'],
            },
            {
              name: 'Viola Cornuta Colormax',
              description: 'Horned pansy, available in various colours',
              price: 'Price on request',
              season: ['spring', 'summer'],
            },
            {
              name: 'Cosmos Bipinnatus Sensation',
              description: 'Large, airy flowers — ideal for bouquets',
              price: 'Price on request',
              season: ['spring', 'summer'],
            },
            {
              name: 'Gaillardia Grandiflora Arizona',
              description: 'Perennial with warm tones, available in multiple colours',
              price: 'Price on request',
              season: ['spring', 'summer'],
            },
            {
              name: 'Geranium Zonal Fantasia',
              description: 'Compact geranium, available in multiple colours',
              price: 'Price on request',
              season: ['spring', 'summer'],
            },
            {
              name: 'Geranium Zonal Galaxy',
              description: 'Geranium with bicolour decorative flowers',
              price: 'Price on request',
              season: ['spring', 'summer'],
            },
          ],
        },
        {
          id: 'dehydrated',
          emoji: '🍃',
          name: 'Dehydrated Essentials',
          tag: 'available',
          limited: true,
          defaultSeason: 'all-seasons',
          description: 'Transformed with love and passion — indispensable in the kitchen.',
          detail: 'Dehydrated at low temperature to preserve all nutrients and flavour. Their shelf life is naturally long-lasting.',
          items: [
            {
              name: 'Pépites d\'or — Dried Garlic',
              description: 'Garlic dehydrated at low temperature to preserve nutrients',
              use: 'All-purpose cooking',
              price: '$6',
              volume: '237 ml',
              image: '/images/vegetable/Pepite_d_or_seche.jpg',
            },
            {
              name: 'Pépites d\'or — Dried Garlic with Herbs & Chili',
              description: 'Dried garlic blended with dried herbs and chili',
              use: 'Marinades, dressings, spicy dishes',
              price: '$6',
              volume: '237 ml',
              image: '/images/vegetable/Pépite_d_or_ail_séché_aux_herebes_et_piment.jpg',
            },
            {
              name: 'Hot Chili Blend',
              description: 'Dehydrated jalapeño',
              use: 'Spicy cooking',
              price: '$5',
              volume: '237 ml',
              image: '/images/vegetable/Mélange_de_piments_foret.jpg',
            },
            {
              name: 'THE Blend',
              description: 'All-purpose seasoning with garlic, sun-dried tomatoes and garden herbs, with a hint of chili',
              use: 'Meats, vegetables, pasta — all-purpose',
              price: '$6',
              volume: '237 ml',
              image: '/images/vegetable/LE_mélange.jpg',
            },
          ],
        },
        {
          id: 'frozen',
          emoji: '❄️',
          name: 'Frozen Essentials',
          tag: 'available',
          defaultSeason: 'all-seasons',
          description: 'Carefully Prepared for you — indispensable in the kitchen.',
          detail: 'Blanched, chilled then frozen at peak freshness to preserve every nutrient and flavour. Convenient and delicious for your everyday cooking.',
          items: [
            {
              name: 'Soup Blend',
              description: 'Carrot, onion, sweet potato, celery, corn',
              use: 'Soups & potages',
              price: '$5',
              volume: '500 g',
              image: '/images/vegetable/Congelé_Mélange_a_soupe.jpg',
            },
            {
              name: 'Spaghetti Blend',
              description: 'Carrot, onion, pepper, celery, garlic',
              use: 'Pasta sauces',
              price: '$5',
              volume: '500 g',
              image: '/images/vegetable/Congelé_sauce_a_spag.jpg',
            },
            {
              name: 'Ratatouille Blend',
              description: 'Pepper, zucchini, onion, eggplant, tomato, garlic',
              use: 'Ratatouille, stews',
              price: '$5',
              volume: '500 g',
              image: '/images/vegetable/Congelé_ratatouille.jpg',
            },
            {
              name: 'All-Purpose Blend',
              description: 'Onion, pepper, beans, squash, garlic',
              use: 'All-purpose cooking',
              price: '$5',
              volume: '500 g',
              image: '/images/vegetable/Congelé_passe_partout.jpg',
            },
            {
              name: 'Vegetable Broth',
              description: 'Gently infused with vegetables and fine herbs',
              use: 'Soups, risottos, sauces',
              price: '$5',
              volume: '946 ml',
              image: '/images/vegetable/Bouillon_de_légumes.jpg',
            },
            {
              name: 'Herb Pesto',
              description: 'Blend of herbs with nuts, parmesan, oil and garlic',
              use: 'Pasta, crostini, marinades',
              price: '$6',
              volume: '237 ml',
              image: '/images/vegetable/Pesto.jpg',
            },
            {
              name: 'Garlic Butter',
              description: 'Butter infused with garden garlic',
              use: 'Bread, vegetables, grilled meats',
              price: '$6',
              volume: '237 ml',
              image: '/images/vegetable/Beurre_a_l_ail.jpg',
            },
            {
              name: 'Tomato Sauce Base',
              description: 'Homemade tomato sauce with fresh vegetables',
              use: 'Pasta, pizzas, stews',
              price: '$8',
              volume: '946 ml',
              image: '/images/vegetable/Sauce_tomate.jpg',
            },
            {
              name: 'Tomato & Pepper Sauce Base',
              description: 'Tomato sauce enriched with garden peppers',
              use: 'Pasta, pizzas, stews',
              price: '$8',
              volume: '946 ml',
              image: '/images/vegetable/Sauce_tomate_poivrons.jpg',
            },
          ],
        },
        {
          id: 'seasonal',
          emoji: '🥕',
          name: 'Seasonal Vegetables',
          tag: 'available',
          limited: true,
          description: 'No pesticides or chemical fertilizers.',
          detail: 'Fresh vegetables harvested at peak ripeness. Delivery within 5 km of Senneville and Sainte-Anne-de-Bellevue, $5 delivery fee applies, minimum order of $30. On-site pickup also available.',
          items: [
            {
              name: 'Fresh Bag of the Day',
              description: 'A blend of vegetables, greens and freshly harvested herbs',
              use: 'Meals throughout the week',
              price: '$20',
              season: ['summer', 'fall'],
              image: '/images/vegetable/Sac_de_légumes_frais.jpg',
            },
            {
              name: 'Garlic Scape',
              description: 'Seasonal — spring',
              use: 'Sautéed, fermented vegetables, pesto',
              price: '$5 per pound',
              season: 'spring',
              image: '/images/vegetable/Fleur_d_ail.jpg',
            },
            {
              name: 'Fresh Garlic',
              description: 'Garden garlic harvested in summer',
              use: 'All-purpose cooking',
              price: 'Contact us',
              season: 'summer',
              image: '/images/vegetable/Ail_frais.jpg',
            },
            {
              name: 'End-of-Season Tomatoes',
              description: 'Ripe tomatoes harvested in autumn',
              use: 'Sauce, preserves, fresh',
              price: 'Contact us',
              season: 'fall',
              image: '/images/vegetable/Tomates_de_fin_de_saison.jpg',
            },
          ],
        },
        {
          id: 'prepared',
          emoji: '🍲',
          name: 'PP\'s Kitchen — Prepared Meals',
          tag: 'soon',
          description: 'This category is coming soon.',
          detail: 'We are working on this new offering: home-cooked meals made with our very own vegetables. Stay tuned for delicious updates!',
          items: [],
        },
        {
          id: 'misc',
          emoji: '✨',
          name: 'Miscellaneous',
          tag: 'available',
          defaultSeason: 'all-seasons',
          description: 'Natural amendment to enrich your soil.',
          detail: 'Complementary products to care for your garden.',
          items: [
            {
              name: 'Biochar',
              description: 'Long-lasting soil amendment to enrich and condition your garden',
              use: 'Gardening, soil improvement',
              price: '$1 per litre',
              image: '/images/vegetable/Biochar.jpg',
              link: 'https://lasemaineverte.com/biochar',
            },
          ],
        },
      ],
    },

    about: {
      title: 'About Me',
      subtitle: 'The woman behind Le Pré Paré',
      name: 'Chantal Paré',
      role: 'Founder & Producer',
      bio: [
        'Holder of a certificate in horticulture and green space management, I have specialized for over 20 years in the creation of vegetable gardens and in the planning and maintenance of all types of plants.',
        'Passionate about gardening, nutrition, and cooking, I have long dreamed of a project that brings all of my passions together in one place.',
        'My past experiences have brought me a wealth of knowledge, led me to meet wonderful people, and allowed me to participate in various enriching projects.',
        'Today I want to share what makes me happy and offer an experience bold enough to awaken your taste buds.',
        'We produce in an ecological and responsible way.',
      ],
      values: {
        title: 'Our Values',
        items: [
          { icon: '🌎', title: 'Ecological & Responsible', text: 'Production without pesticides or chemical fertilizers, in harmony with nature and the environment.' },
          { icon: '❤️', title: 'Passion & Love', text: 'Every gesture and care given to our plants and vegetables reflects our deep love for the land and healthy food.' },
          { icon: '🤝', title: 'Community', text: 'Meeting wonderful people and sharing our passion with our local community.' },
          { icon: '⭐', title: 'Quality', text: 'Offering the best value through fresh, local, and carefully grown products — straight from the garden.' },
        ],
      },
      contact: {
        title: 'Let\'s stay in touch',
        cta: 'Write to us',
      },
    },

    contact: {
      title: 'Contact Us',
      subtitle: 'We\'re here for any question, comment, order, or collaboration.',
      info: {
        title: 'Information',
        address: 'Address',
        addressValue: '25 Legault Street, Senneville, QC',
        phone: 'Phone',
        phoneValue: '514-691-0949',
        email: 'Email',
        emailValue: 'le.prepare1@gmail.com',
        social: 'Social Media',
        payment: 'Payment',
        paymentValue: 'Interac e-Transfer',
      },
      form: {
        title: 'Send Us a Message',
        name: 'Your name',
        email: 'Your email',
        subject: 'Subject',
        message: 'Your message',
        send: 'Send message',
        namePlaceholder: 'Marie Tremblay',
        emailPlaceholder: 'marie@example.com',
        subjectPlaceholder: 'Question about products…',
        messagePlaceholder: 'Hello Chantal, I would like to…',
        successMsg: 'Thank you! Your message has been sent.',
      },
    },

    footer: {
      tagline: 'From my garden to your table.',
      quickLinks: 'Quick Links',
      contactTitle: 'Contact',
      rights: 'All rights reserved.',
      madeWith: 'Made with ❤️ in Québec',
    },
  },
}
